...
# ------------------------------------------------------------------ tab_accounts
ACC = {"span erasure, frozen bases": ("magnitude", "targeted"), "energy-matched random subspace": ("magnitude", "random_1_matched"),
...
m02 = ct.split("### M02")[1].split("### M03")[0]
for m in MODELS:
    mm = re.search(re.escape(NAME[m]) + r": D = ([+-]\d\.\d+) \[([+-]\d\.\d+), ([+-]\d\.\d+)\] \(R = ([+-]\d+\.\d)%\)", m02)
    k = srow(m, "cure_centred_svd_r1_a1")
    if not mm:
        fail("contract M02 line for %s not parsed" % NAME[m]); continue
    if not same(float(mm.group(1)), k.D, 3) or not same(float(mm.group(4)), 100 * k.R, 1):
        fail("contract M02 vs data %s: D %s vs %s, R %s vs %s" % (NAME[m], mm.group(1), k.D, mm.group(4), 100 * k.R))
m06 = ct.split("### M06")[1].split("### M07")[0]
for m in MODELS:
    mm = re.search(re.escape(NAME[m]) + r": erasure r1 MMLU (\d\.\d+) -> (\d\.\d+), ppl (\d+\.\d) -> (\d+\.\d)", m06)
    if not mm:
        fail("contract M06 line for %s not parsed" % NAME[m]); continue
    u, e = probe(m, "unedited_r0_a0", "mmlu_200", "value"), probe(m, "cure_centred_svd_r1_a1", "mmlu_200", "value")
    pu, pe = probe(m, "unedited_r0_a0", "wikitext2_20k", "perplexity"), probe(m, "cure_centred_svd_r1_a1", "wikitext2_20k", "perplexity")
    if not (same(float(mm.group(1)), u, 3) and same(float(mm.group(2)), e, 3) and same(float(mm.group(3)), pu, 1) and same(float(mm.group(4)), pe, 1)):
        fail("contract M06 vs data %s: %s vs %s" % (NAME[m], mm.groups(), (u, e, pu, pe)))
print("Evidence Contract M02 and M06 agree with the raw files")
